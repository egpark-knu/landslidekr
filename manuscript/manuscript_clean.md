...
Code availability. The full LandslideKR orchestrator code, per-event configurations, analysis scripts, executed traces, and the manuscript source are publicly available at <https://github.com/egpark-knu/landslidekr>. The repository includes the runtime fallback chain for topographic preprocessing (richdem $\to$ pysheds $\to$ numpy), the year-match NIDR filter documented in §2.3, and the offline recomputation and figure-generation scripts referenced in §3 (`scripts/run_case.py`, `scripts/run_benchmarks.py`, `scripts/apply_far_filters.py`, `scripts/axis3_stochastic_repeatability.py`, `scripts/axis4_param_sensitivity.py`, `scripts/axis6_label_ablation.py`, `scripts/calibration_isotonic.py`, `scripts/post_hoc_refinement.py`, and the `scripts/make_fig*.py` figure generators). A pinned archive of the submission-associated release will be deposited to Zenodo with a DOI upon acceptance.
